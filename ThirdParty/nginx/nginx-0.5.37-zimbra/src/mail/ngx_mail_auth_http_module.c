...
static void ngx_mail_auth_http_recv_routelookup_response_handler(ngx_event_t *rev);
static void ngx_mail_auth_http_process_routelookup_response(ngx_mail_session_t *s, 
    ngx_mail_auth_http_ctx_t *ctx);
static void ngx_mail_auth_http_process_routelookup_reponse_headers(ngx_mail_session_t *s,
    ngx_mail_auth_http_ctx_t *ctx);
static void ngx_mail_auth_http_routelookup_connect_handler(ngx_event_t *ev);
static void ngx_mail_auth_http_wait_handler(ngx_event_t *rev);
...
static void *ngx_mail_auth_http_create_conf
    (ngx_conf_t *cf);
static char *ngx_mail_auth_http_merge_conf
    (ngx_conf_t *cf, void *parent, void *child);
static char *ngx_mail_auth_http
    (ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
static char *ngx_mail_auth_http_header
    (ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
...
static void reelect_auth_server
    (ngx_mail_auth_http_ctx_t *ctx, ngx_mail_auth_http_conf_t *cnf);
static inline ngx_peer_addr_t *elected_peer
    (ngx_mail_auth_http_ctx_t *ctx, ngx_mail_auth_http_conf_t *cnf);
...
static void
ngx_mail_auth_http_recv_routelookup_response_handler(ngx_event_t *rev)
{
    ssize_t                    n, size;
    ngx_connection_t          *c;
    ngx_mail_session_t        *s;
...
next:
    ...
    ctx->state = sw_start;
    ctx->route_response_handler =
            ngx_mail_auth_http_process_routelookup_reponse_headers;
    ctx->route_response_handler(s, ctx);
}


static void
ngx_mail_auth_http_process_routelookup_reponse_headers(ngx_mail_session_t *s,
    ngx_mail_auth_http_ctx_t *ctx)
{
    u_char                      *p;
    time_t                       timer;
        ...
        if (rc == NGX_OK) {
                   ...
                   == 0)
            {
                ...
                else
                {
                    ...
                    p = ngx_palloc(s->connection->pool, size);
                    if (p == NULL) {
                        ngx_mail_auth_http_dispose_context (s);
                        ngx_mail_session_internal_server_error(s);
                        return;
...
static void
ngx_mail_auth_http_wait_handler(ngx_event_t *rev)
{
    ngx_connection_t          *c;
    ngx_mail_session_t        *s;
...
ngx_mail_auth_http_create_conf(ngx_conf_t *cf)
{
    ...
    if (ngx_array_init (&ahcf->peers, cf->pool, 4, sizeof (ngx_peer_addr_t *))
        != NGX_OK)
    {
        return NULL;
    }
...
ngx_mail_auth_http(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    {
        /* Make space for one (ngx_peer_addr_t *) */
...
static inline ngx_peer_addr_t *
elected_peer (ngx_mail_auth_http_ctx_t *ctx, ngx_mail_auth_http_conf_t *cnf)
{
    return ((ngx_peer_addr_t **)cnf->peers.elts)[ctx->url_index];
}
...
/* memcache handler (NOOP)
 */
static void mch_noop (mc_work_t *w)
{
    /* NOOP */
}
