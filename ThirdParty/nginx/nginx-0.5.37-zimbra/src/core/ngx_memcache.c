...
static void *ngx_memcache_create_conf(ngx_cycle_t *cycle)
{
    ...
    if(ngx_array_init (&mcf->servers, mcf->cpool, 4, sizeof(ngx_peer_addr_t*))
        != NGX_OK) {
        return NGX_CONF_ERROR;
    }
...
static ngx_int_t ngx_memcache_init_process(ngx_cycle_t *cycle)
{
    ...
    peers = (ngx_peer_addr_t **)mcf->servers.elts;
    ...
    )
{
    ...
    while (t < pdu.len)
    {
        ...
        if (n > 0) {
            ...
            ngx_log_error (NGX_LOG_NOTICE, l, 0,
                    "channel %V orderly shutdown when posting request",
                    mcctx->srvconn->name);
...
/* this pseudo handler is invoked for all cache responses 
   which arrive from the memcached connection after the stipulated
   timeout 
 */
static void mc_noop_handler (mc_work_t *w)
{
...
static void ngx_memcache_any_read_handler(ngx_event_t *rev)
{
    ...
    while ((!ngx_memcache_wq_isempty (wq_head)) && ((readbuffer->last - readbuffer->pos) > 0))
    {
        ...
        ngx_str_t temp_readbuffer;
        temp_readbuffer.data = readbuffer->pos;
        temp_readbuffer.len = readbuffer->last - readbuffer->pos;

        ngx_log_debug2 (NGX_LOG_DEBUG_ZIMBRA, ngx_cycle->log, 0,
        		"start to process memcache response (len=%d)\n%V",
        		temp_readbuffer.len, &temp_readbuffer);
