...
main(int argc, char *const *argv)
{
    ...
    if (ngx_show_version) {
        rc=ngx_write_fd(ngx_stderr_fileno, "nginx version: " NGINX_VER CRLF,
                     sizeof("nginx version: " NGINX_VER CRLF) - 1);

        if (ngx_show_configure) {
#ifdef NGX_COMPILER
            rc=ngx_write_fd(ngx_stderr_fileno, "built by " NGX_COMPILER CRLF,
            ...
            rc=ngx_write_fd(ngx_stderr_fileno,
