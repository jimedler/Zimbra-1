...
static void
ngx_http_upstream_noop_ip_hash_stage (ngx_http_request_t *r)
{
    /* noop */
    return;
}
    ...
    ngx_http_upstream_srv_conf_t *us, ngx_http_upstream_stage_pt stage)
{
    ...
    if (ngx_http_upstream_init_round_robin_peer
        (r, us, ngx_http_upstream_noop_ip_hash_stage) != NGX_OK) {
        return NGX_ERROR;
    }
    ...
    for ( ;; ) {
        ...
        if (++iphp->tries >= 20) {
            ngx_log_debug0 (NGX_LOG_DEBUG_HTTP, pc->log, 0,
                "ip_hash: iphash tries>20, fallback to round robin");
            return iphp->get_rr_peer(pc, &iphp->rrp) == NGX_OK ? NGX_OK :
