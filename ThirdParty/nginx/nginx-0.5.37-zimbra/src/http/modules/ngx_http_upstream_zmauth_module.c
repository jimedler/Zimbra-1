...
static ngx_flag_t ngx_http_upstream_zmserver_from_cookie 
    (ngx_log_t *log, ngx_pool_t *pool, ngx_table_elt_t *cookie, ngx_peer_addr_t *peer);
    ...
    for (;;)
    {
        ...
        if (rc == NGX_DONE)
        {
            ...
            if (ctx->addr.len == 0 || ctx->port.len == 0) {
                ...
                zmauth_cache_route(r, ausr, route);   /* cache http route for future use */
    ...
    (ngx_http_request_t *r, ngx_str_t usr, ngx_str_t qusr)
{
    ...
    //? Why use memcache's ttl to do zmauth things
    ttl = mcf->ttl / 1000;
    if (mcf->ttl % 1000 != 0) {
        ++ttl;
    }
