...
enum ngx_http_proxy_location_type_e {
    ngx_http_proxy_location_text,
    ngx_http_proxy_location_vars,
    ngx_http_proxy_location_regex
};

typedef enum ngx_http_proxy_location_type_e ngx_http_proxy_location_type_t;
...
typedef union {
    ...
    void        *regex;
} ngx_http_proxy_location_header_t;

struct ngx_http_proxy_redirect_s
{
    ngx_http_proxy_redirect_pt     handler;

    ngx_http_proxy_location_type_t      lhstype;
    ngx_http_proxy_location_header_t    lhs;

    ngx_http_proxy_location_type_t      rhstype;
    ngx_http_proxy_location_header_t    rhs;
};
...
/* Generic rewrite handler for all types of Location header fudging */
static ngx_int_t
ngx_http_proxy_rewrite_redirect_any(ngx_http_request_t *r, ngx_table_elt_t *h,
    size_t prefix, ngx_http_proxy_redirect_t *pr)
{
    ...
    /* L=LHS
       R=RHS
       H=actual location header
       N=rewritten location header
     */
    H = h->value;

    /* Blow LHS if necessary */
    if (pr->lhstype == ngx_http_proxy_location_text) {
        L = pr->lhs.text;
    } else {
        L.len = 0;
        ngx_memzero(&e,sizeof(ngx_http_script_engine_t));
        e.ip = pr->lhs.vars.lengths;
        e.request = r;
        ...
        L.data = ngx_palloc(r->pool,L.len);
        if (L.data == NULL) {
            return NGX_ERROR;
        }

        p = L.data;
        e.ip = pr->lhs.vars.values;
        e.pos = p;
    ...
    ngx_log_debug1 (NGX_LOG_DEBUG_HTTP, r->connection->log, 0, 
        "proxy rewrite any: computed LHS %V", &L);

    /* Make a basic check here
       No fudge will occur here if any of the following holds true

       > if L is longer than H
         eg: we are searching for http://verylongname.com/ and we get 
             Location: http://short.com/

       > if L is not longer than H, but H does not begin with L
         eg: we are searching for http://foo.com/ and we get 
             Location: http://bar.com/xxx/yyy/zzz


       If we decide not to fudge, then computing RHS is unnecessary
     */

    if (L.len > H.len - prefix ||
        ngx_rstrncmp(H.data+prefix,L.data,L.len) != 0
       )
    {
        return NGX_DECLINED;
    }

    /* Blow RHS if necessary */
    if (pr->rhstype == ngx_http_proxy_location_text) {
        R = pr->rhs.text;
    } else {
        R.len = 0;
        ngx_memzero(&e,sizeof(ngx_http_script_engine_t));
        e.ip = pr->rhs.vars.lengths;
        e.request = r;
        ...
        R.data = ngx_palloc(r->pool,R.len);
        if (L.data == NULL) {
            return NGX_ERROR;
        }

        p = R.data;
        e.ip = pr->rhs.vars.values;
        e.pos = p;
    ...
    /* now compute the replacement */

    N.len = prefix + (H.len - L.len) + R.len;  /* XXX:+prefix required here ??? */
    N.data = ngx_palloc(r->pool,N.len);
    ...
    p = N.data;
    p = ngx_cpymem(p,H.data,prefix);
    p = ngx_cpymem(p,R.data,R.len);
    p = ngx_cpymem(p,H.data+prefix+L.len,H.len-prefix-L.len);

    h->value = N;

    ngx_log_debug4 (NGX_LOG_DEBUG_HTTP, r->connection->log, 0, 
        "proxy rewrite any: LHS:%V, RHS:%V, IN:%V, OUT:%V",
        &L, &R, &H, &N);
...
ngx_http_proxy_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->redirect) {
        ...
        if (conf->redirects == NULL && conf->upstream.url.data) {
            ...
            pr->handler = ngx_http_proxy_rewrite_redirect_any;
            pr->lhs.text = conf->upstream.url;
            pr->lhstype = ngx_http_proxy_location_text;

            pr->rhstype = ngx_http_proxy_location_text;
            if (conf->upstream.uri.len) {
                pr->rhs.text = conf->upstream.location;
            } else {
                pr->rhs.text.len = 0;
                pr->rhs.text.data = NULL;
            }
        }
    }
...
/* This method handles the "proxy_redirect" configuration directive
 */
static char *
ngx_http_proxy_redirect(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    /* If the argument to proxy_redirect is "default", it means that NGINX will
       replace all the Location headers beginning with the Upstream location 
       (http://$upstream_name) with the URL of the presently active location

       Otherwise, proxy_redirect takes two arguments (referred to here as LHS 
       and RHS), which indicate the pattern to be searched and the pattern to 
       replace.
     */
    ...
    if (cf->args->nelts == 2 && ngx_strcmp(value[1].data, "default") == 0)
    {
        ...
        pr->lhs.text = plcf->upstream.url;
        pr->lhstype = ngx_http_proxy_location_text;

        if (plcf->upstream.uri.len) {
            pr->rhs.text = plcf->upstream.location;
            pr->rhstype = ngx_http_proxy_location_text;
    ...
    /* Now we need to see how many variables are present in LHS, and in RHS */
    ...
    else
    {
        ...
        {
            /* There are some HTTP variables in the LHS */
            ngx_memzero(&sc, sizeof(ngx_http_script_compile_t));
            ...
            pr->lhs.vars.lengths = vars_lengths->elts;
            pr->lhs.vars.values = vars_values->elts;
            pr->lhstype = ngx_http_proxy_location_vars;
        }
        ...
        {
            /* There are some HTTP variables in the RHS */
            ngx_memzero(&sc, sizeof(ngx_http_script_compile_t));
            ...
            pr->rhs.vars.lengths = vars_lengths->elts;
            pr->rhs.vars.values = vars_values->elts;
            pr->rhstype = ngx_http_proxy_location_vars;
        }
    }

    /* Since we know lhstype and rhstype, one generic rewrite handler suffices
       for all cases (handler:ngx_http_proxy_rewrite_redirect_any)
     */
    pr->handler = ngx_http_proxy_rewrite_redirect_any;
    return NGX_CONF_OK;
}
