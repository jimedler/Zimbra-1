...
static void
ngx_http_upstream_prepconnect(ngx_http_request_t *r);
...
ngx_http_upstream_init(ngx_http_request_t *r)
{
    ...
    if (u->conf->upstream->peer.init
        (r, u->conf->upstream, ngx_http_upstream_prepconnect) != NGX_OK) {
        ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }
}
...
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (!u->buffering) {
        ...
        if (clcf->tcp_nodelay && c->tcp_nodelay == NGX_TCP_NODELAY_UNSET) {
            ...
            {
                ngx_connection_error(c, ngx_socket_errno == NGX_EINVAL ?
                    NGX_ECONNRESET : ngx_socket_errno,
