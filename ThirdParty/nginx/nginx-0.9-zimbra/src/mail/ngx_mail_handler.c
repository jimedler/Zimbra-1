...
#include <ngx_mail_pop3_module.h> /* to be refactor */
#include <ngx_mail_smtp_module.h> /* to be refactor */
 ...
 */
ngx_int_t
ngx_mail_auth_plain(ngx_mail_session_t *s, ngx_connection_t *c)
{
    u_char     *p, *last;
    ngx_str_t  *args, *arg, plain, temp;
    ngx_uint_t narg;
    args = s->args.elts;
    narg = s->args.nelts;
    arg = args + narg - 1;
    ...
    if (ngx_decode_base64(&plain, arg) != NGX_OK) {
...
ngx_mail_auth_login_username(ngx_mail_session_t *s, ngx_connection_t *c)
{
    ...
    arg = args + narg - 1;
    ngx_log_debug1(NGX_LOG_DEBUG_MAIL, c->log, 0,
                   "mail auth login username: \"%V\"", arg);
    ...
    s->login.data = ngx_pnalloc(c->pool, ngx_base64_decoded_length(arg->len));
    if (s->login.data == NULL){
        return NGX_ERROR;
    }

    if (ngx_decode_base64(&s->login, arg) != NGX_OK) {
        ngx_log_error(NGX_LOG_INFO, c->log, 0,
...
ngx_mail_auth_login_password(ngx_mail_session_t *s, ngx_connection_t *c)
{
...
#if (NGX_DEBUG_MAIL_PASSWD)
    ngx_log_debug1(NGX_LOG_DEBUG_MAIL, c->log, 0,
                   "mail auth login password: \"%V\"", arg);
#endif
    /* check if the auth exchange is being aborted */
    if (narg > 0 &&
        arg->len == 1 &&
    ...
    s->passwd.data = ngx_pnalloc(c->pool,
                                ngx_base64_decoded_length(arg->len));
    if (s->passwd.data == NULL){
        return NGX_ERROR;
    }

    if (ngx_decode_base64(&s->passwd, arg) != NGX_OK) {
        ngx_log_error(NGX_LOG_INFO, c->log, 0,
                      "client sent invalid base64 encoding in AUTH LOGIN command");
        return NGX_MAIL_PARSE_INVALID_COMMAND;
    }
...
ngx_mail_log_error(ngx_log_t *log, u_char *buf, size_t len)
{
    ...
    n = ngx_sock_ntop((struct sockaddr *)dw_peer, buf, len, 1);
    len -= n;
    buf += n;
    ...
    *buf++ = '-';
    ...
    n = ngx_sock_ntop((struct sockaddr *)up_host, buf, len, 1);
    len -= n;
    buf += n;
