    ...
    (ngx_str_t *ip, throttle_callback_t *callback)
{
    ...
    l = sizeof("add ") - 1 +
        ...
        sizeof(" ") - 1 +
        sizeof("0 ") - 1 +
        tscf->mail_login_ip_ttl_text.len +
        sizeof(" ") - 1 +
    ...
    p = ngx_sprintf (pdu->data, "add %V 0 %V 1" CRLF "1" CRLF,
            &k, &tscf->mail_login_ip_ttl_text);
    ...
    (ngx_str_t *user, throttle_callback_t *callback)
{
    ...
    l = sizeof("add ") - 1 +
        ...
        sizeof(" ") - 1 +
        sizeof("0 ") - 1 +
        tscf->mail_login_user_ttl_text.len +
        sizeof(" ") - 1 +
        sizeof("1") - 1 + sizeof(CRLF) - 1 +
        1 + sizeof(CRLF) - 1;
    ...
    p = ngx_sprintf (pdu->data, "add %V 0 %V 1" CRLF "1" CRLF,
            &k, &tscf->mail_login_user_ttl_text);
