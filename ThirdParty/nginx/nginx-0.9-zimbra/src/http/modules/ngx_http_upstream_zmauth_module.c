...
static ngx_flag_t zmauth_elect_routelookup_server
    (ngx_http_upstream_zmauth_ctx_t *ctx, ngx_http_upstream_zmauth_srv_conf_t *zscf);

static void zmauth_lookup_route(ngx_http_request_t *r);
static void zmauth_lookup_fqdn_from_memcache(ngx_http_request_t *r);
static void zmauth_lookup_fqdn_from_memcache_success_handler(mc_work_t *w);
static void zmauth_lookup_fqdn_from_memcache_failure_handler(mc_work_t *w);
static void zmauth_lookup_route_from_memcache(ngx_http_request_t *r);
static void zmauth_lookup_route_from_memcache_success_handler (mc_work_t *w);
static void zmauth_lookup_route_from_memcache_failure_handler (mc_work_t *w);
static void zmauth_lookup_route_from_server (ngx_http_request_t *r);
static ngx_int_t zmauth_make_routelookup_request(ngx_http_request_t *r);
static void zmauth_send_routelookup_request_handler(ngx_event_t *wev);
static void zmauth_recv_routelookup_response_handler(ngx_event_t *rev);
static void zmauth_process_routelookup_response(ngx_http_request_t *r);
static void zmauth_process_routelookup_response_headers(ngx_http_request_t *r);
static ngx_int_t zmauth_parse_routelookup_response_headers(ngx_http_request_t *r);
static ngx_int_t zmauth_retrieve_route (ngx_pool_t * pool, ngx_str_t * addr_text,
...
static void zmauth_cache_fqdn
    (ngx_http_request_t *r, ngx_str_t usr, ngx_str_t qusr);
static void zmauth_cache_route
    (ngx_http_request_t *r, ngx_str_t usr, ngx_str_t route);
static void zmauth_cache_fqdn_handler (mc_work_t *w);
static void zmauth_cache_route_handler (mc_work_t *w);
...
static void ngx_http_upstream_zmauth_cleanup (void * data)
{
    ...
    if(ctx != NULL) {
        if (ctx->wait_memcache) {
            ngx_memcache_ignore_work_by_ctx(r);
        }
    ...
    ngx_http_upstream_srv_conf_t *us, ngx_http_upstream_stage_pt stage)
{
    ...
    if (zmp->zmroutetype != zmroutetype_fallback)
    {
        ...
        ctx->stage = stage;
        ctx->tries = 0;
        zmauth_elect_routelookup_server(ctx, zscf);
...
/* The portal to lookup upstream's route. first lookup from memcache,
 * and if not found, find in "route lookup server"
 */
static void zmauth_lookup_route(ngx_http_request_t *r)
{
    ngx_http_upstream_zmauth_ctx_t          *ctx;
    ...
    if (zmp->zmroutetype == zmroutetype_authtoken) {
        //don't lookup alias for auth token
        ngx_log_debug1(NGX_LOG_DEBUG_HTTP, log, 0,
            "zmauth: learning route by id:%V",
            &ctx->qusr);
        zmauth_lookup_route_from_memcache(r);
        return;
    }

    zmauth_lookup_fqdn_from_memcache (r);
}

/*
 * Consider current user name is an alias and lookup its
 * corresponding Full Qualified Domain Name (fqdn), such
 * as 'john' --> 'john@test.com'
 */
static void zmauth_lookup_fqdn_from_memcache (ngx_http_request_t * r)
{
    ngx_http_upstream_zmauth_ctx_t          *ctx;
    ...
    if (k.len == 0) {    /* NOMEM */
        return zmauth_lookup_route_from_server(r);
    }

    w.ctx = r;
    w.request_code = mcreq_get;
    w.response_code = mcres_unknown;
    w.on_success = zmauth_lookup_fqdn_from_memcache_success_handler;
    w.on_failure = zmauth_lookup_fqdn_from_memcache_failure_handler;
...
static void zmauth_lookup_route_from_memcache (ngx_http_request_t *r)
{
    ...
    if (k.len == 0) {   /* NOMEM */
        zmauth_lookup_route_from_server(r);
        return;
    }
    ...
    w.request_code = mcreq_get;
    w.response_code = mcres_unknown;
    w.on_success = zmauth_lookup_route_from_memcache_success_handler;
    w.on_failure = zmauth_lookup_route_from_memcache_failure_handler;
...
static void zmauth_lookup_route_from_memcache_success_handler (mc_work_t *w)
{
    ...
    route_text.data = ngx_pstrdup(r->pool, &w->payload);
    ...
    if (route_text.data == NULL) {
        ...
        if (ngx_parse_url(r->pool, &u) == NGX_ERROR) {
            zmauth_lookup_route_from_server(r);
        }

        zmp->zmpeer.sockaddr = ngx_palloc(r->pool, u.socklen);
        if (zmp->zmpeer.sockaddr == NULL) {
           zmauth_lookup_route_from_server(r);
        }

        ngx_memcpy(zmp->zmpeer.sockaddr, u.sockaddr, u.socklen);
        zmp->zmpeer.socklen = u.socklen;
        zmp->zmpeer.name = route_text;
...
/* lookup route from "route lookup server" */
static void zmauth_lookup_route_from_server(ngx_http_request_t *r)
{
    ngx_http_upstream_zmauth_srv_conf_t         *zscf;
    ngx_http_upstream_zmauth_ctx_t              *ctx;
    ...
    if (rc == NGX_ERROR || rc == NGX_BUSY || rc == NGX_DECLINED) {
        ...
        if (!zmauth_elect_routelookup_server(ctx, zscf)) {
            ngx_log_error(NGX_LOG_ERR,r->connection->log, 0,
...
static void zmauth_routelookup_connect_handler(ngx_event_t *ev)
{
    ...
    if(sockerr == EINPROGRESS) {
        ...
        if (zmauth_make_routelookup_request(r) != NGX_OK) {
            zmauth_dispose_context(r);
            zmauth_error(r);
            return;
        }
        ...
        ctx->peer.connection->write->handler = zmauth_send_routelookup_request_handler;
        ctx->peer.connection->read->handler = zmauth_recv_routelookup_response_handler;
        ctx->route_response_handler = zmauth_process_routelookup_response;
...
static void zmauth_process_routelookup_response(ngx_http_request_t *r)
{
    ...
    for (p = ctx->route_resp->pos; p < ctx->route_resp->last; p++)
    {
        ch = *p;
...
next:
    ...
    p = ctx->route_resp->start - 1;

done:

    ctx->route_resp->pos = p + 1;
    ctx->state = 0;
    ctx->route_response_handler = zmauth_process_routelookup_response_headers;
    ctx->route_response_handler (r);
}

static ngx_int_t
zmauth_parse_routelookup_response_headers(ngx_http_request_t *r)
{
    u_char      c, ch, *p;
    enum {
        sw_start = 0,
...
static void zmauth_process_routelookup_response_headers(ngx_http_request_t *r)
{
    ...
    for (;;)
    {
        rc = zmauth_parse_routelookup_response_headers(r);
        ...
        if (rc == NGX_DONE)
        {
            ...
            if (zmauth_retrieve_route (r->pool,
                    &ctx->addr, &ctx->port, &zmp->zmpeer) == NGX_ERROR) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
            ...
            zmauth_cache_fqdn(r, ctx->usr, ausr); /* cache http user alias */
            zmauth_cache_route(r, ausr, zmp->zmpeer.name);   /* cache http route for future use */
...
static ngx_int_t
zmauth_retrieve_route (ngx_pool_t * pool, ngx_str_t * addr_text,
            ngx_str_t * port_text, ngx_addr_t * route)
{
    struct sockaddr         *sa;
    struct sockaddr_in      *sin;
...
static void zmauth_send_routelookup_request_handler(ngx_event_t *wev)
{
    ...
    size = ctx->route_req->last - ctx->route_req->pos;
    n = ngx_send(c, ctx->route_req->pos, size);
...
static void zmauth_recv_routelookup_response_handler(ngx_event_t *rev)
{
    ...
    if(ctx->route_resp == NULL) {
        ctx->route_resp = ngx_create_temp_buf(ctx->pool, 1024);
        /* TODO handle NULL */
    }

    size = ctx->route_resp->end - ctx->route_resp->last;
    n = ngx_recv(c, ctx->route_resp->pos, size);

    if(n > 0) {
        ctx->route_resp->last += n;
        ctx->route_response_handler (r);
        return;
    }
...
static ngx_int_t zmauth_make_routelookup_request(ngx_http_request_t *r)
{
    ...
    pool = ctx->pool;
    zmp = ctx->zmp;
    auth_method = (zmp->zmroutetype == zmroutetype_authtoken ?
                    NGX_HTTP_ID_AUTHMETHOD : NGX_HTTP_DEFAULT_AUTHMETHOD);
    vhost = r->headers_in.host->value;

    len = sizeof("GET ") - 1 + uri.len + sizeof(" HTTP/1.0" CRLF) - 1
        + sizeof("Host: ") - 1 + hh.len + sizeof(CRLF) - 1
        + sizeof("Auth-Method: ") - 1 + auth_method.len + sizeof(CRLF) - 1
    ...
    b->last = ngx_cpymem(b->last, auth_method.data, auth_method.len);
    b->last = ngx_cpymem(b->last, CRLF, sizeof(CRLF) - 1);
...
/* cache the username mapping as the fqdn reported by lookup servlet (Auth-User) */
static void zmauth_cache_fqdn
    (ngx_http_request_t *r, ngx_str_t usr, ngx_str_t qusr)
{
    mc_work_t                                w;
    ...
    p = ngx_sprintf (pdu.data, "add %V 0 %V %d" CRLF "%V" CRLF,
                        &k, &mcf->ttl_text, qusr.len, &qusr);
    ...
    w.response_code = mcres_unknown;
    w.on_success = zmauth_cache_fqdn_handler;
    w.on_failure = zmauth_cache_fqdn_handler;
    ...
    (ngx_http_request_t *r, ngx_str_t usr, ngx_str_t route)
{
    ...
    p = ngx_sprintf(pdu.data, "add %V 0 %V %d" CRLF "%V" CRLF,
                    &k, &mcf->ttl_text, route.len, &route);
    ...
    w.request_code = mcreq_add;
    w.response_code = mcres_unknown;
    w.on_success = zmauth_cache_route_handler;
    w.on_failure = zmauth_cache_route_handler;
