...
public abstract class Encoding {
     ...
     * will return <code>off + 2</code> (UTF-16) or <code>off + 3</code> 
     * (UTF-8), otherwise it will return <code>off</code>
     *
     * @see TextDecl
     * @see XmlDecl
     * @see #TOK_XML_DECL
    ...
    Encoding getInitialEncoding(byte[] buf, int off, int end, Token token) {
        ...
        switch (end - off) {
            ...
            switch ((b0 << 8) | b1) {
                ...
                case 0xEFBB:
                    if ((end - off) >= 3 && (buf[off + 2] & 0xFF) == 0xBF)
    ...
    Encoding getEncoding(String name) {
        ...
        if (name.equalsIgnoreCase("US-ASCII") || name.equalsIgnoreCase("ASCII"))
            return getEncoding(ASCII_ENCODING);
        return SingleByteEncodings.get(name);
    }
